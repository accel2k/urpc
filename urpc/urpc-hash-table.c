...
uRpcHashTable *
urpc_hash_table_create (urpc_hash_table_destroy_callback value_destroy_func)
{
...
void
urpc_hash_table_destroy (uRpcHashTable *hash_table)
{
  HashNode *node;
  uint32_t i;

  if (hash_table->type != URPC_HASH_TABLE_TYPE)
    return;
...
int
urpc_hash_table_insert (uRpcHashTable *hash_table,
                        uint32_t       key,
                        void          *value)
{
  ...
  if (parrent == NULL)
    hash_table->nodes[key % HASH_TABLE_SIZE] = node;
  else
    parrent->next = node;
  hash_table->nnodes += 1;
...
int
urpc_hash_table_insert_uint32 (uRpcHashTable *hash_table,
                               uint32_t       key,
                               uint32_t       value)
{
...
void *
urpc_hash_table_find (uRpcHashTable *hash_table,
                      uint32_t       key)
{
  HashNode *node;
...
uint32_t
urpc_hash_table_find_uint32 (uRpcHashTable *hash_table,
                             uint32_t       key)
{
...
void
urpc_hash_table_foreach (uRpcHashTable                   *hash_table,
...
uint32_t
urpc_hash_table_size (uRpcHashTable *hash_table)
{
  if (hash_table->type != URPC_HASH_TABLE_TYPE)
    return 0;
...
int
urpc_hash_table_remove (uRpcHashTable *hash_table,
                        uint32_t       key)
{
  HashNode *parrent = NULL;
  HashNode *node;
      ...
      if (node->key == key)
        {
          if (parrent != NULL)
            ...
            hash_table->nodes[key % HASH_TABLE_SIZE] = node->next;
