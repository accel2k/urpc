...
uRpcMemChunk *urpc_mem_chunk_create( int chunk_size )
{
  ...
  if( mem_chunk == NULL ) return NULL;

  // Смещение до данных в каждой странице. Занято заголовком страницу ChunkPage.
  // Смещение кратно DATA_ALIGN.
  mem_chunk->offset = DATA_ALIGN * ( ( sizeof( ChunkPage ) / DATA_ALIGN ) + ( sizeof( ChunkPage ) % DATA_ALIGN ? 1 : 0 ) );
  ...
  if( mem_chunk->pages == NULL ) { free( mem_chunk ); return NULL; }
...
void urpc_mem_chunk_destroy( uRpcMemChunk *mem_chunk )
{
  ...
  if( mem_chunk->type != URPC_MEM_CHUNK_TYPE ) return;
...
void *urpc_mem_chunk_alloc( uRpcMemChunk *mem_chunk )
{
  ...
  if( mem_chunk->type != URPC_MEM_CHUNK_TYPE ) return NULL;
  ...
  if( i >= mem_chunk->nchunks_in_page ) return NULL;
...
int urpc_mem_chunk_free( uRpcMemChunk *mem_chunk, void *chunk )
{
  ...
  if( mem_chunk->type != URPC_MEM_CHUNK_TYPE ) return -1;
