...
static void *urpc_tcp_server_func( void *data )
{
  ...
  while( !urpc_tcp_server->shutdown )
    ...
    if( urpc_tcp_server->cur_clients == urpc_tcp_server->max_clients ) continue;
...
uRpcTCPServer *urpc_tcp_server_create( const char *uri, uint32_t threads_num, uint32_t max_clients, uint32_t max_data_size, double timeout )
{
  ...
  if( max_clients > FD_SETSIZE ) return NULL;
  ...
  if( urpc_tcp_server == NULL ) return NULL;
  ...
  if( urpc_tcp_server->urpc_data == NULL ) goto urpc_tcp_server_create_fail;
  for( i = 0; i < threads_num; i++ ) urpc_tcp_server->urpc_data[i] = NULL;
  for( i = 0; i < threads_num; i++ )
    {
    urpc_tcp_server->urpc_data[i] = urpc_data_create( max_data_size, sizeof( uRpcHeader ), NULL, NULL, 0 );
    if( urpc_tcp_server->urpc_data[i] == NULL ) goto urpc_tcp_server_create_fail;
    }

  // Таймаут таймеры.
  urpc_tcp_server->timers = malloc( threads_num * sizeof( uRpcTimer* ) );
  if( urpc_tcp_server->timers == NULL ) goto urpc_tcp_server_create_fail;
  for( i = 0; i < threads_num; i++ ) urpc_tcp_server->timers[i] = NULL;
  for( i = 0; i < threads_num; i++ )
    {
    urpc_tcp_server->timers[i] = urpc_timer_create();
    if( urpc_tcp_server->timers[i] == NULL ) goto urpc_tcp_server_create_fail;
    }
  ...
  if( urpc_tcp_server->wsockets_per_threads == NULL ) goto urpc_tcp_server_create_fail;
  for( i = 0; i < threads_num; i++ ) urpc_tcp_server->wsockets_per_threads[i] = INVALID_SOCKET;

  // Адрес сервера.
  addr = urpc_get_sockaddr( uri );
  if( addr == NULL ) goto urpc_tcp_server_create_fail;
  ...
  if( urpc_tcp_server->connector == NULL ) goto urpc_tcp_server_create_fail;
...
void urpc_tcp_server_destroy( uRpcTCPServer *urpc_tcp_server )
{
  ...
  if( urpc_tcp_server->urpc_tcp_server_type != URPC_TCP_SERVER_TYPE ) return;
  ...
  if( urpc_tcp_server->lsocket != INVALID_SOCKET ) closesocket( urpc_tcp_server->lsocket );
  ...
  if( urpc_tcp_server->timers != NULL )
    ...
    for( i = 0; i < urpc_tcp_server->threads_num; i++ )
      if( urpc_tcp_server->timers[i] != NULL ) urpc_timer_destroy( urpc_tcp_server->timers[i] );
  ...
  if( urpc_tcp_server->urpc_data != NULL )
    {
    for( i = 0; i < urpc_tcp_server->threads_num; i++ )
      if( urpc_tcp_server->urpc_data[i] != NULL ) urpc_data_destroy( urpc_tcp_server->urpc_data[i] );
...
uRpcData *urpc_tcp_server_recv( uRpcTCPServer *urpc_tcp_server, uint32_t thread_id )
{
  ...
  if( urpc_tcp_server->urpc_tcp_server_type != URPC_TCP_SERVER_TYPE ) return NULL;
  if( thread_id > urpc_tcp_server->threads_num - 1 ) return NULL;
  ...
  if( urpc_tcp_server->cur_clients == 0 )
    ...
    for( i = 0; i < urpc_tcp_server->max_clients; i++ )
      ...
      if( urpc_tcp_server->wsockets[i] != INVALID_SOCKET )
        ...
        if( urpc_tcp_server->wsockets[i] > max_fd ) max_fd = urpc_tcp_server->wsockets[i];
        }
      }
    }
  ...
  // Нет запросов.
  if( urpc_tcp_server->wsockets_per_threads[ thread_id ] == INVALID_SOCKET ) return NULL;
...
int urpc_tcp_server_send( uRpcTCPServer *urpc_tcp_server, uint32_t thread_id )
{
  ...
  if( thread_id > urpc_tcp_server->threads_num - 1 ) return -1;

  wsocket = urpc_tcp_server->wsockets_per_threads[ thread_id ];
  urpc_tcp_server->wsockets_per_threads[ thread_id ] = INVALID_SOCKET;
  if( wsocket == INVALID_SOCKET ) return -1;
...
SOCKET urpc_tcp_server_get_client_socket( uRpcTCPServer *urpc_tcp_server, uint32_t thread_id )
{
  ...
  if( urpc_tcp_server->urpc_tcp_server_type != URPC_TCP_SERVER_TYPE ) return INVALID_SOCKET;
  if( thread_id > urpc_tcp_server->threads_num - 1 ) return INVALID_SOCKET;
